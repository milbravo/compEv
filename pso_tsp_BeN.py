...
def bellmore_nemhauser_tsp(distance_matrix):
    num_cities = len(distance_matrix)
    unvisited = list(range(num_cities))  # Lista para armazenar vértices nao visitados
    visited=list()  # Lista para armazenar vértices visitados
    ...
    while len(visited) < num_cities:
        ...
        # Encontrar a próxima cidade conectada
        next_city=int
        next_city_distance=distance_matrix[current_city][current_city]
        for i in unvisited:
          aux_distance=distance_matrix[current_city][i]
          #Atualiza cidade mais próxima
          if(aux_distance<next_city_distance):
            next_city_distance=distance_matrix[current_city][i]
            next_city=i
        visited.append(next_city)
        unvisited.remove(next_city)

    return visited, calculate_distance(visited, distance_matrix)

# Algoritmo PSO para o TSP
def pso_tsp(distance_matrix, num_particles,W, C1, C2, max_iterations, max_trocas):
    num_cities = len(distance_matrix)
    save_every=10000
    save_file="PSO_results_BeN.json"
    # Inicializar partículas (permutação aleatória das cidades)
    particles = [ bellmore_nemhauser_tsp(distance_matrix)[0] for _ in range(num_particles)]
